...
**Solution:** Persist hiding in the returned `StrikerState.ball_hiding` field. Enter hide when `dist >= START`; stay hidden until `dist < END`, then aim. `main.py` / `simulate.py` must feed the previous controller state back in each call.
